...
func (session *Session) Receive(msg interface{}, msg_id *uint16) (err error) {
	if session.IsClosed() {
		return ErrClosed
	}
	err = session.decoder.Decode(msg, msg_id)
	if err != nil {
		session.Close()
	}
	return
...
func (session *Session) Send(msg interface{}, args ...interface{}) (err error) {
	if session.IsClosed() {
		return ErrClosed
	}

	//session.conn.SetWriteDeadline(time.Now().Add(time.Millisecond * 250))
	err = session.encoder.Encode(msg, args...)
	if err != nil {
		session.Close()
		println("read package error")
	}
	return
}
